...
void* create_user_kern_stack(uint32* ptr_user_page_directory)
{
	...
	void* va = kmalloc(KERNEL_STACK_SIZE);

	uint32 *ptr_table;
	get_page_table(ptr_user_page_directory, (uint32)va, &ptr_table);
	ptr_table[PTX(va)] = ptr_table[PTX(va)] & ~PERM_PRESENT;
...
void initialize_uheap_dynamic_allocator(struct Env* e, uint32 daStart, uint32 daLimit)
{
	...
	e->segment_break = ROUNDDOWN(daStart, PAGE_SIZE);//not garbage
